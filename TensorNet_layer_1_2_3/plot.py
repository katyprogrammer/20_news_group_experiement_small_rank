...
def parse_arg():
    parser = optparse.OptionParser('usage%prog [-i infile] [-o outfile] [-A all] [-r A->B or B->A] [-i num]')
    ...
    parser.add_option('-i', dest='num', default='0')
    (options, args) = parser.parse_args()
    return options

def plot(fin,fout,title,isB=False, num):
    global Base
    ...
    plt.xlabel('epoch')
    plt.savefig('loss_{0}_{1}.png'.format(fout, i))
    plt.clf()
    if isB:
